...
ROOT = Path(__file__).resolve().parents[2] / "physicsMDSR_Range_CSV"
SUMMARY_NAME = "pysr_warm_0_7_summary.csv"

GROUP_IDS = tuple(range(8))       # 0.csv ... 7.csv
MIN_COMPLEXITY = 7
MAX_COMPLEXITY = 35
FORMULA_LIMIT = 30                # lowest-loss formulas within the range
MAXFEV = 200_000
...
# True -> recompute even if result columns already contain values.
# Resume mode:
# False = rows that already have finite MSE/R2 are not recomputed.
# This is the recommended setting after an interrupted/partially completed run.
FORCE_RECOMPUTE = False

# Skip an entire summary file before submitting it to a worker when every row
# already has a finite shared_fit_mse_0_7 and shared_fit_r2_0_7.
PROCESS_ONLY_INCOMPLETE = True
...
def build_parser():
    parser = argparse.ArgumentParser(description="Refit and rank generalized PySR formulas across dataset groups")
    parser.add_argument("data_root", type=Path, help="Root containing per-ID PySR summaries and datasets")
    parser.add_argument("--summary-name", default=SUMMARY_NAME)
    parser.add_argument("--datasets", type=parse_dataset_indices, default=GROUP_IDS)
    parser.add_argument("--workers", type=int, help="Parallel folder workers; default uses available CPUs")
    parser.add_argument("--min-complexity", type=int, default=MIN_COMPLEXITY)
    parser.add_argument("--max-complexity", type=int, default=MAX_COMPLEXITY)
    parser.add_argument("--formula-limit", type=int, default=FORMULA_LIMIT)
    parser.add_argument("--maxfev", type=int, default=MAXFEV)
    parser.add_argument("--force", action="store_true", help="Recompute rows with existing metrics")
    parser.add_argument("--include-redundant", action="store_true")
    return parser


def configure(args):
    global ROOT, SUMMARY_NAME, GROUP_IDS, MAX_WORKERS
    global MIN_COMPLEXITY, MAX_COMPLEXITY, FORMULA_LIMIT, MAXFEV
    global FORCE_RECOMPUTE, SKIP_REDUNDANT
    if args.workers is not None and args.workers <= 0:
        raise ValueError("--workers must be positive")
    if min(args.formula_limit, args.maxfev) <= 0 or args.min_complexity > args.max_complexity:
        raise ValueError("invalid complexity, formula-limit, or maxfev configuration")
    ROOT = args.data_root.expanduser().resolve()
    SUMMARY_NAME = args.summary_name
    GROUP_IDS = tuple(args.datasets)
    ...
    SKIP_REDUNDANT = not args.include_redundant
...
def process_summary(summary_file: Path):
    ...
    # Select the lowest-loss candidates within the requested complexity range.
