...
4. writes them to noise_<K>_MSE, noise_<K>_NMSE, and noise_<K>_R2; and
...
def wass_directory_name(wass_label: str) -> str:
    """Map a metric label such as WASS_001 to its data folder 001."""
    return wass_label[5:] if wass_label.upper().startswith("WASS_") else wass_label
...
def resolve_wass_root(data_root: Path, wass_label: str) -> Path:
    ...
    directory_name = wass_directory_name(wass_label)
    if data_root.name.lower() in {
        directory_name.lower(),
        wass_label.lower(),
    }:
        return data_root
    return data_root / directory_name
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "data_root", type=Path,
        help=(
            "Data root containing value-named folders such as 001, 01, 003, "
            "and 005, or one such folder directly"
        ),
    )
    parser.add_argument("--top-rank", type=int, required=True, help="Process only rows whose TopRank equals this value")
    parser.add_argument(
        "--noise", type=normalize_wass_label, nargs="+",
        default=["WASS_0", "WASS_025", "WASS_04"],
        help=(
            "One or more noise values, for example: --noise 001 01 003 005. "
            "Data folders use the raw values and metric columns use noise_<value>"
        ),
    )
    parser.add_argument(
        "--output", type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    wass_labels = tuple(args.noise)
    if len(wass_labels) != len(set(wass_labels)):
        raise FitDataError(f"--noise contains duplicate values: {wass_labels}")
    ...
    print(
        ...
        "Noise=" + ", ".join(wass_directory_name(label) for label in wass_labels)
    )
    if inferred_summary:
